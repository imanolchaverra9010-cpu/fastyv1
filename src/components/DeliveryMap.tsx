...
const DeliveryMap = ({ pickup, dropoff, courier }: Props) => {
  ...
  const validPoints = [pickup, dropoff, ...(courier ? [courier] : [])].filter(
    (p): p is { lat: number; lng: number } => Boolean(p && !isNaN(p.lat) && !isNaN(p.lng))
  );
  ...
  return (
    <div className="relative h-full w-full rounded-2xl overflow-hidden bg-muted flex flex-col border border-border">
      <MapContainer center={[validPoints[0].lat, validPoints[0].lng]} zoom={13} style={{ height: "100%", width: "100%" }} zoomControl={false}>
        <TileLayer
        ...
        {routeCoords.length > 0 && (
          <Polyline
            ...
            weight={5}
            opacity={0.8}
            dashArray={courier ? "1, 10" : undefined}
          />
        )}
      </MapContainer>
      ...
      {courier && (
        <div className="absolute top-4 left-4 z-10 bg-white/95 backdrop-blur-sm px-4 py-2 rounded-full shadow-md border border-border flex items-center gap-3">
          ...
          <span className="text-xs font-bold text-foreground">{courier.label || "Domiciliario en camino"}</span>
        </div>
      )}
    </div>
  );
